...
shuff_reduced <- subset(x=ref_data_context, 
                subset=model_type=='shuffled' & target_type=='reduced')
shuff_unreduced <- subset(x=ref_data_context, 
                        subset=model_type=='shuffled' & target_type=='unreduced')

order_reduced <- subset(x=ref_data_context, 
                        subset=model_type=='ordered' & target_type=='reduced')
order_unreduced <- subset(x=ref_data_context, 
                          subset=model_type=='ordered' & target_type=='unreduced')
...
# rel
t.test(shuff_reduced$rel_surp, order_reduced$rel_surp)
t.test(shuff_reduced$rel_surp, mu=0)
t.test(order_reduced$rel_surp, mu=0)

t.test(shuff_unreduced$rel_surp, order_unreduced$rel_surp)
t.test(shuff_unreduced$rel_surp, mu=0)
t.test(order_unreduced$rel_surp, mu=0)
...
order_r <- order_reduced$main_surp
order_u <- order_unreduced$main_surp
...
plt2 +labs(title="Surprisal of Garden Path between Contexts",
           x ="Training Method", y = "Surprisal of One NP minus Two NP")
